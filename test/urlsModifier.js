...
describe("Check script urlsModifier.js: ", function () {
  ...
  describe("Check class RuleConfigurator: ", function () {
    ...
    function isAllowedRuleTypeDetected(ruleConfiguratorInstance, ruleType) {
      try {
        ...
        if (
          exception.name === ModuleUrlsModifier.RuleTypeInvalidExceptionName
        ) {
          return false;
        } else {
          throw exception;
    ...
    it("Check function set ruleType use deobfuscate option: ", function () {
      assert.equal(
        (ruleConfigurator.ruleType = ruleTypes.ruleDeobfuscate),
        ruleTypes.ruleDeobfuscate,
      );
    });
    it("Check function set ruleType use obfuscate option: ", function () {
      assert.equal(
        (ruleConfigurator.ruleType = ruleTypes.ruleObfuscate),
        ruleTypes.ruleObfuscate,
      );
    });
  ...
  describe("Check class RuleTransformations: ", function () {
    ...
    it("Check class: ", function () {
      ...
      assert.equal(
        ruleTransformations.ruleTransformations[0].valueNew,
        mockRuleValuesGeneral[0].valueNew,
      );
      assert.equal(
        ruleTransformations.ruleTransformations[0].valueOld,
        mockRuleValuesGeneral[0].valueOld,
      );
    });
    ...
    it("Check function isThereAnyRule: ", function () {
      ...
      const ruleTransformationsNoRule =
        new ModuleUrlsModifier.RuleTransformations([], []);
  ...
  describe("Check class RulesApplicator: ", function () {
    it("Check function modifyUrls, with rules: ", function () {
      ...
      const rulesApplicator = new ModuleUrlsModifier.RulesApplicator(
        ruleTransformations,
      );
  ...
  describe("Check class Rules: ", function () {
    ...
    it("Check function addTypeAndRule: ", function () {
      ...
      assert.equal(
        rules.ruleTransformationsToUse,
        ruleTransformations.ruleTransformations,
      );
    });
    ...
    it("Check function deleteRule: ", function () {
      ...
      assert.equal(
        rules.ruleTransformationsToUseStringRepresentation,
        new ModuleUrlsModifier.RuleTransformations(["a1", "c1"], ["a2", "c2"])
          .stringRepresentation,
      );
    });
    it("Check function updateRule: ", function () {
      ...
      rules.updateRule(getRule("b1", "b2"), getRule("c1", "c2"));
      assert.equal(
        rules.ruleTransformationsToUseStringRepresentation,
        new ModuleUrlsModifier.RuleTransformations(["a1", "c1"], ["a2", "c2"])
          .stringRepresentation,
    ...
    it("Check Rules._ruleConfigurator._ruleTypes: ", function () {
      assert.equal(
        String(rules.ruleTypes),
        String(new ModuleUrlsModifier.RuleTypes().ruleTypes),
      );
    });
  ...
  describe("Check function getUrlsWithPaths: ", function () {
    it("Check function getUrlsWithPaths", function () {
      assert.equal(
        String(["http://github.com/CarlosAMolina", "http://github.com"]),
        String(
          ModuleUrlsModifier.getUrlsWithPaths([
  ...
  describe("Check constant PROTOCOL_DEFAULT: ", function () {
    it("Check constant PROTOCOL_DEFAULT: ", function () {
      assert.equal("http://", ModuleUrlsModifier.PROTOCOL_DEFAULT);
    });
  });
});
